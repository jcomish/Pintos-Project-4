...
#include <list.h> /* For the sleep list */
...
bool order_by_wake_time(const struct list_elem *a,
						const struct list_elem *b,
						void *aux)
{
	return (list_entry(a, struct thread, elem)->waketime < 
	        list_entry(b, struct thread, elem)->waketime);
}
...
timer_sleep (int64_t ticks) 
{
  ...
  ASSERT (intr_get_level () == INTR_ON);
  //setup an end time and go to block state
  ...
  list_insert_ordered(&sleep_list, &t->elem, &order_by_wake_time, NULL);
  thread_block(); // No longer yielding but blocking
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  if (cur % RECALC_FREQ == 0)
//	thread_set_mlfq_priority_for_all();
